...
def _SingleDump(moduleName, excludeNames):
	...
	print("	cmd: " + "lua.exe sprotodump.lua -cs {0} -d {1} -p {2}".format(" ".join(protoFiles), outDir, moduleName))
	os.system("lua.exe sprotodump.lua -cs {0} -d {1} -p {2}".format(" ".join(protoFiles), outDir, moduleName))
